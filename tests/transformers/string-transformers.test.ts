...
const cases: TestCase[] = [
    ...
    ['some_value_here', 'someValueHere', 'some-value-here', 'some_value_here'] as const,
    ['alreadyCamelCase', 'alreadyCamelCase', 'already-camel-case', 'already_camel_case'] as const,
    ['Already_Pascal_Case', 'alreadyPascalCase', 'already-pascal-case', 'already_pascal_case'] as const,
];
...
describe.each(cases)('StringTransformers (%s)', (snake, camel, kebab, expectedSnake) => {
    test('toCamelCase', () => {
    ...
    ObjectUtils.entries(guards).forEach(([name, guard]) => {
        test(`${name} correctly identifies valid strings`, () => {
            const value = name === 'isCamelCase' ? camel :
                name === 'isKebabCase' ? kebab :
                    expectedSnake;
            expect(guard(value)).toBe(true);
        });

        test(`${name} correctly rejects invalid strings`, () => {
            const invalidValues = ['', null, undefined, 123, 'Hello World', 'some-value!', 'UPPERCASE'];
