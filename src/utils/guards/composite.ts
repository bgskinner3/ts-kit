...
export const isUrlOrRelativePath: TTypeGuard<string> = (
    ...
    if (value.startsWith('/')) return true;     // Relative URLs starting with '/'
    return isValidUrl(value);     // Otherwise must be a valid absolute URL
};
...
): value is Record<string, V> => isObject(value) && ObjectUtils.values(value).every(typeGuard)

/**
 * Type guards for primitive types (string, number, boolean, etc.)
 * @see {@link ValidationUtilsDocs.CompositeTypeGuards}
 */
